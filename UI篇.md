...
1. 在规定的16.7ms之内，下一桢VSync信号到来之前，并没有CPU和GPU共同完成下一帧图形的画面的合成，就会造成卡顿和掉帧

2. 滑动优化方案：

   1. CPU：对象创建、调整、销毁；预排版（布局计算、文本计算）；预渲染（文本等异步绘制，图片编解码等）

   2. GPU：纹理渲染；视图混合

### 绘制原理和异步绘制

1. `[layer.delegate displaylayer]` 代理负责生成对应的bitmap；设置该bitmap座位layer.contents属性的值

2. 离屏渲染

   1. 当我们处理图层的属性在被制定未被合并之前不能直接在屏幕上显示，就出发了离屏渲染，离屏渲染的概念起源预GPU层图，指的是GPU在当前屏幕缓冲区意外新开辟了一个缓冲区进行渲染操作
   ...
   3. 为何要避免？
      ...
      3. 出发丽萍渲染会增加GPU的工作量，而增加了GPU的工作量就很有可能导致CPU和GPU的工作总耗时超过16.7ms，就有可能到时UI的卡顿和掉帧
